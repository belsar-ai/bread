...
├── data/
│   ├── org.bread.policy                  # Polkit policy
│   ├── bread.desktop                     # Desktop launcher
│   ├── bread-snapshot.service            # Systemd service
│   └── bread-snapshot.timer              # Systemd timer
└── bread.spec                            # RPM spec

Installed layout:
/usr/bin/bread                            # Auto-generated by console_scripts
/usr/bin/bread-gui                        # Auto-generated by gui_scripts
/usr/lib/python3.*/site-packages/bread/   # Package
/usr/lib/systemd/system/bread-snapshot.*  # RPM-packaged
/usr/share/polkit-1/actions/org.bread.policy
/usr/share/applications/bread.desktop
/etc/systemd/system/mnt-_bread.mount      # Created by bread config at runtime
```
...
[project.scripts]
bread = "bread.cli.main:main"

[project.gui-scripts]
bread-gui = "bread.gui.app:main"
```

---

### 4. Dispatcher (`bread/cli/main.py`)
...
### 6. CLI: Setup Wizard (`bread/cli/config.py`)
...
### 7. CLI: Snapshot (`bread/cli/snapshot.py`)
...
### 8. CLI: Rollback (`bread/cli/rollback.py`)
...
### 9. CLI: Revert (`bread/cli/revert.py`)
...
### 10. CLI: Purge (`bread/cli/purge.py`)
...
**GUI entry point (`bread/gui/app.py`):**

`gi.require_version` calls must happen before any Gtk/Adw imports, so they
live at module top-level. `main()` is the gui-scripts entry point.
...
### 12. Polkit Policy (`/usr/share/polkit-1/actions/org.bread.policy`)
...
### 13. Desktop File (`/usr/share/applications/bread.desktop`)
